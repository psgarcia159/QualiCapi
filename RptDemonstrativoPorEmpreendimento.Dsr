VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} RptDemonstrativoPorEmpreendimento 
   ClientHeight    =   9630
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   11460
   OleObjectBlob   =   "RptDemonstrativoPorEmpreendimento.dsx":0000
End
Attribute VB_Name = "RptDemonstrativoPorEmpreendimento"
...
Dim VFV_TOTALMOEDADEBITO As New XArray
...
Private Sub Report_Initialize()
  ...
  VFV_SOMATORIO.ReDim 0, 0, 0, 2  'colunas = quantidade de campos para soma +1 +1 chave
  VFV_SOMATORIO2.ReDim 0, 0, 0, 2  'colunas = quantidade de campos para soma +1 +1 chave
  VFV_VALORES.ReDim 0, 0, 0, 1 'colunas = quantidade de campos para soma +1
...
Private Sub SecDetalhe_Format(ByVal pFormattingInfo As Object)
  ...
  If XLO_CAMPOS.GetItemByName("titu_dt_Pagamento").Value = "" Then
    ...
    If XFT_TITULOANTERIOR <> XLO_CAMPOS.GetItemByName("Titulo").Value And XFB_IMPRIMIU = False Then
      SubSomaMoeda2 XLI_MOEDA, XLT_MOEDA, VFV_TOTALMOEDADEBITO, XLF_VALORTITULO, Left(XLO_CAMPOS.GetItemByName("Titulo").Value, 4)
    End If
  ...
  Else
    ...
    If XFT_TITULOANTERIOR <> XLO_CAMPOS.GetItemByName("Titulo").Value And XFB_IMPRIMIU = False Then
      If FormulaFields.GetItemByName("IndexadorExtra").Value <> "" Then
        SubSomaMoeda2 FormulaFields.GetItemByName("IndexadorExtra").Value, FormulaFields.GetItemByName("NomeIndexadorExtra").Value, VFV_TOTALMOEDALIQUIDADO, XLF_VALORPAGOINDEXADO, Left(XLO_CAMPOS.GetItemByName("Titulo").Value, 4)
      Else
        SubSomaMoeda2 XLI_MOEDA, XLT_MOEDA, VFV_TOTALMOEDALIQUIDADO, XLF_VALORPAGOINDEXADO, Left(XLO_CAMPOS.GetItemByName("Titulo").Value, 4)
      End If
    End If
  ...
  SetReportVariableValue "SOMAVALORPAGO", (FunNuloVal(GetReportVariableValue("SOMAVALORPAGO")) + XLF_VALORPAGO)
...
Private Sub SecFooterCliente1_Format(ByVal pFormattingInfo As Object)
  ...
  If XFB_IMPRIMIU = False Then
    ...
    VFV_VALORES(0, 1) = GetReportVariableValue("SOMAVALORPAGO")
      ...
      If VFV_SOMATORIO(XFI_CONT, 0) = Left(XFT_TITULOANTERIOR, 12) Then
        While XFI_CONT < VFV_SOMATORIO.Count(1)
          ...
          Else
            ...
            TxtSomaValorPago.SetText Format(VFV_SOMATORIO(XFI_CONT, 2), "standard")
            XFI_CONT = XFI_CONT + 1
          End If
        Wend
      End If
...
Private Sub SecFooterEmpreendimento1_Format(ByVal pFormattingInfo As Object)
  ...
  If XFB_IMPRIMIU = True Then
    ...
    For XFI_CONT = 0 To VFV_TOTALMOEDADEBITO.Count(1) - 1
      If VFV_TOTALMOEDADEBITO(XFI_CONT, 3) = Left(XFT_TITULOANTERIOR, 4) Then
    ...
    For XFI_CONT = 0 To VFV_TOTALMOEDALIQUIDADO.Count(1) - 1
      If VFV_TOTALMOEDALIQUIDADO(XFI_CONT, 3) = Left(XFT_TITULOANTERIOR, 4) Then
...
Private Sub SecFooterEmpreendimento2_Format(ByVal pFormattingInfo As Object)

  Dim XFI_CONT As Integer
  
  'Enquanto não chegou no final do relatorio guarda o soma por moeda no vetor
  If XFB_IMPRIMIU = False Then
    VFV_VALORES(0, 0) = GetReportVariableValue("TOTALDEBITOATUALIZADO")
    VFV_VALORES(0, 1) = GetReportVariableValue("TOTALLIQUIDADOATUALIZADO")
    
    SubSomatorioVetor VFV_SOMATORIO2, VFV_VALORES, Left(XFT_TITULOANTERIOR, 4)
  
  'Depois que já passou pelo fim do reltório descarrega o vetor com os somatórios
  Else
    For XFI_CONT = 0 To VFV_SOMATORIO2.Count(1) - 1
      If VFV_SOMATORIO2(XFI_CONT, 0) = Left(XFT_TITULOANTERIOR, 4) Then
        While XFI_CONT < VFV_SOMATORIO2.Count(1)
          If VFV_SOMATORIO2(XFI_CONT, 0) <> Left(XFT_TITULOANTERIOR, 4) Then
            XFI_CONT = VFV_SOMATORIO2.Count(1) + 1
          Else
            TxtTotalDebitoAualizado.SetText FunTrataZero(Format(FunNuloVal(VFV_SOMATORIO2(XFI_CONT, 1)), "standard"))
            TxtTotalLiquidadoAtualizado.SetText FunTrataZero(Format(FunNuloVal(VFV_SOMATORIO2(XFI_CONT, 2)), "standard"))
            XFI_CONT = XFI_CONT + 1
          End If
        Wend
      End If
