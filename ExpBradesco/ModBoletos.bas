...
' Obs.: Os requisitos para uso da biblioteca Boleto2Net como COM devem ser vistos
...
Public Function FunGetAuthorization() As Boolean
    ...
    HttpReq.Open "POST", UrlToken, False
    ...
    ' - Anexa o Certificado à requisição
    ' --------------------------------------------------------------------------------------------
...
Public Function FunPostBoleto(Payload As String) As String
    ...
    ' - Carrega a URL
    ' --------------------------------------------------------------------------------------------
    UrlBoleto = XLO_BOLETOS.Item(XLT_ROOTITEM).Item("Endpoints").Item("UrlBase")
    
    ' - Cria o objeto HTTP POST Request
    ' --------------------------------------------------------------------------------------------
    ' HttpReq.Open "POST", XLO_BOLETOS.Item(XLT_ROOTITEM).Item("Endpoints").Item("UrlBase"), False
    HttpReq.Open "POST", UrlBoleto, False
    ...
    HttpReq.setRequestHeader "Connection", "keep-alive"
    HttpReq.setRequestHeader "Content-Length", Len(Payload)
...
Public Function RemoverCaracteresEspeciais(ByVal Texto As String) As String
    ...
    ' Adicione ou remova caracteres desta lista conforme necessário
    Dim CaracteresParaRemover As String
    Dim j As Integer
    CaracteresParaRemover = "!@#$%¨&*()_+{}[]|\:;<>,.?/°ºª"
