...
Begin VB.Form TelaInicial 
   Caption         =   "Pagamento Eletrônico - REAL"
   ClientHeight    =   5370
   ClientLeft      =   765
   ClientTop       =   1800
   ClientWidth     =   7440
...
Private Sub Form_Load()
    ...
    XGT_NomeAplic = "EXPREAL"
    XGT_IdAplic = "EXPREAL-LAERPXE"
    subTestaProtecao XGT_NomeAplic, XGT_IdAplic, XGT_DirPadrao
    XGT_NomeAplic = "EXPREAL"
    LblEmpresa.Caption = PFantasia
    TelaInicial.Label1.Caption = "R " & Format(App.Major, "#0") & "." & _
    Format(App.Revision, "00")
    
    '------------------------------------------------------------------------
    'ATUALIZAÇÃO DO SISTEMA
    
    Diretorio = Mid(App.Path, 1, (Len(App.Path) - Len("\QualiCapi\ExpReal")))
    
    'Verifica a versão do cliente no banco de dados
    SubQOpenRecordset XLO_VERIFICA, "SELECT sist_vl_versao FROM Sistema WHERE sist_tx_nome = 'QUALICAPI - EXPREAL'", Estatico
    ...
    If Not XLO_VERIFICA.BOF Then
        ...
        If CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) > CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            Conexao.Execute _
                ...
                " WHERE sist_tx_nome = 'QUALICAPI - EXPREAL'"
        ...
        ElseIf CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) < CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            res = MsgBox("Máquina desatualizada! Deseja atualizar o ExpReal nesta máquina?", VbMsgBoxStyle.vbYesNo + vbQuestion, "Aviso")
            
            If (res = vbYes) Then
                Call Shell(Diretorio & "\Atualizador\Atualizador.exe QualiCapi ExpReal", vbNormalFocus)
                End
            Else
