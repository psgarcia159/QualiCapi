...
Begin VB.Form FrmImportacao 
   Caption         =   "Retorno de Pagamento Eletrônico - REAL"
   ClientHeight    =   1785
   ClientLeft      =   2025
   ClientTop       =   1515
   ClientWidth     =   9120
...
Private Sub CmdConfirmar_Click()
  ...
  DlgAbrirFigura.Filter = "Arquivo de Retorno do Real"
  ...
  If Left(XLT_TEXTO, 26) = "02RETORNO01COBRANCA" & Space(7) Then 'Verifica se é o header de retorno
    If Mid(XLT_TEXTO, 77, 13) = "356BANCO REAL" Then 'Verifica se é o do Banco Real
      ...
      While Not EOF(1)
        ...
        If Left(XLT_TEXTO, 1) = "1" Then
          ...
          If Mid(XLT_TEXTO, 19, 12) = XLT_CODEMPRESA Then
        
            'If Mid(XLT_TEXTO, 109, 2) = "02" Or Mid(XLT_TEXTO, 109, 2) = "07" Then
             ...
             If (Mid(XLT_TEXTO, 109, 2) = "98" Or Mid(XLT_TEXTO, 109, 2) = "06" Or Mid(XLT_TEXTO, 109, 2) = "17") And Trim(Mid(XLT_TEXTO, 38, 23)) <> "" Then  ' Amortização
                ...
                If Not IsNull(XLO_IMPORTACAO!moed_cd_Moeda3) Then
                 ...
                 XLF_CORRECAO = XLF_VALORPAGO - XLF_JUROS - XLO_IMPORTACAO!titu_vl_Seguro
                ...
                'Regsitra o log
                Call subRegistraLog("TelaExpBancaria", "2", funCriaDescricaoLog(XGM_MATRIZLOG, IMPORTACAO_CAPI, PAGAMENTO_ELETRONICO_REAL))
    ...
    Else
      MsgBox "Esse não é um arquivo de retorno do Banco Real!"
      XLB_ERRO = True
    End If
...
Private Sub Form_Load()
  ...
  subConectarControleDadosNV DatContaCorrente, "SELECT * FROM ConsGENCCcombo where empr_cd_empresa=" & Int(PCodEmpresa) & " AND banc_cd_codigo=356 ORDER BY coco_tx_Descricao", Estatico
