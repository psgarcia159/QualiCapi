...
'                        "dist_vl_valor=" & FunTrataFloat(XLF_VALOR) & "," & _
'                        "iden_cd_Projeto= " & FunNuloBancoVal(XLO_TITULO!iden_cd_Projeto) & "" & _
'                    " WHERE cere_cd_pcr='" & XLO_TITULO!cere_cd_Pcr & "' and " & "nofi_cd_NotaFiscal = " & Chave

        'Cássio 07/07/2016
        'Removido o filtro centro de custo do where. Esse filtro no where não permitia a substituição do centro de custo para a distribuição.
        XLT_SQL = "UPDATE Distribuicao SET cere_cd_pcr='" & XLO_TITULO!cere_cd_Pcr & "'," & _
                        "plco_cd_conta='" & XLO_TITULO!plco_cd_Conta & "'," & _
                        "dist_vl_valor=" & FunTrataFloat(XLF_VALOR) & "," & _
                        "iden_cd_Projeto= " & FunNuloBancoVal(XLO_TITULO!iden_cd_Projeto) & "" & _
