VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} RptExtratoContratoAnaliticoCliente 
   ClientHeight    =   9465
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   15300
   OleObjectBlob   =   "RptExtratoContratoAnaliticoCliente.dsx":0000
End
Attribute VB_Name = "RptExtratoContratoAnaliticoCliente"
...
Private Sub SecDetalhe_Format(ByVal pFormattingInfo As Object)
  ...
  Else
    ...
    'Cássio 25/07/2016
    'A Costa Andrade solicitou a remoção do deságio para o cálculo do valor previsto
    If PCliente = "Costa Andrade" Then
    ...
    If PCliente = "Costa Andrade" Then
        XLF_VALORPAGO = Format(FunCalculoValorPago(XLO_CAMPOS, 0), "standard")
    Else
  ...
  If (Mid(CStr(XLO_CAMPOS.GetItemByName("Titulo").Value), Len(CStr(XLO_CAMPOS.GetItemByName("Titulo").Value)) - 1, 2) = "00") Then
