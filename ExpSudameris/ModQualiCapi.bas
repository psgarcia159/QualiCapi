  ...
  If XLD_DATABASE < XLD_VENCIMENTO Then
    ...
    If XLT_TIPO = "M" Then 'Mensal
      If XLI_NUMDIAS >= 30 Then
        '30/03/10 - Alteração da fórmula do cálculo de meses. Antes era: XLI_NUMMESES = (XLD_VENCIMENTO - XLD_DATABASE) \ 30
        XLI_NUMMESES = DateDiff("m", XLD_DATABASE, XLD_VENCIMENTO)
        If XLI_NUMMESES < 0 Then XLI_NUMMESES = XLI_NUMMESES * -1
