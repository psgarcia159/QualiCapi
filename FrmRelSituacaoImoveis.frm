...
Begin VB.Form FrmRelSituacaoImoveis 
   ...
   ClientTop       =   3615
   ClientWidth     =   6435
   Icon            =   "FrmRelSituacaoImoveis.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4395
   ...
   Begin VB.Frame FrDistrato 
      Caption         =   "Período Distrato"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
      ...
      Top             =   1560
      Width           =   4710
      Begin MSComCtl2.DTPicker dtDistFim 
         Height          =   315
         Left            =   3000
         TabIndex        =   23
         Top             =   285
      ...
      End
      Begin MSComCtl2.DTPicker dtDistInicio 
         Height          =   315
         Left            =   840
         TabIndex        =   22
         Top             =   285
   ...
   Begin MSDataListLib.DataCombo CboEmpreendimento 
      Bindings        =   "FrmRelSituacaoImoveis.frx":2AFA
      Height          =   315
      Left            =   2160
      TabIndex        =   3
      Top             =   270
   ...
   Begin Threed.SSCommand CmdLimpar 
      ...
      _ExtentY        =   556
      _StockProps     =   78
      MouseIcon       =   "FrmRelSituacaoImoveis.frx":2B1A
      Picture         =   "FrmRelSituacaoImoveis.frx":2B36
   End
...
Private Sub CmdImprimir_Click()
    ...
    Dim XLT_FILTROPERIODODISTRATO As String 'Armazenar filtro do período Distrato  - Daniel Coelho 06/07/2011
    ...
    If FraStatus.Visible Then
    
        'Status do Imóvel
        If ChkVendido.Value = 1 Then
            XGT_SELECAO = XGT_SELECAO & " ((imov_tx_Status='V' AND cont_tx_Status='A' "
            'Período
            ...
            If ChkDisponivel.Value = 1 Then
                XGT_CONJUNCAO = " OR "
                XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & " imov_tx_Status='D' "
                If ChkReservado.Value = 1 Then
                    XGT_CONJUNCAO = " OR "
                    XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & "(imov_tx_Status='R' AND cont_tx_Status='R')) "
            ...
            End If
            XGT_CONJUNCAO = " AND "
        ElseIf ChkDisponivel.Value = 1 Then
            XGT_SELECAO = XGT_SELECAO & " (imov_tx_Status='D' "
            If ChkReservado.Value = 1 Then
                XGT_CONJUNCAO = " OR "
                XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & " (imov_tx_Status='R' AND cont_tx_Status='R')) "
            ...
            XLT_FILTROPERIODO = "Período da Venda: Geral"
        ElseIf ChkReservado.Value = 1 Then
            XGT_SELECAO = XGT_SELECAO & " (imov_tx_Status='R' AND cont_tx_Status='R') "
    ...
    Else
        ...
        End If
         ...
         'Período Distrato - Daniel Coelho - 06/07/2011
         If (Not IsNull(dtDistInicio.Value)) And (Not IsNull(dtDistFim.Value)) Then
            XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & _
            "ConsCAPRelSituacaoImoveis.DtDistrato >= " & FunNuloData(dtDistInicio.Value, NomeSgbd) & "" & _
            " AND ConsCAPRelSituacaoImoveis.DtDistrato <= " & FunNuloData(dtDistFim.Value, NomeSgbd) & ""
            XGT_CONJUNCAO = " AND "
            XLT_FILTROPERIODODISTRATO = "Período de Distrato: " & Format$(dtDistInicio.Value, "dd/mm/yy") & " a " & Format$(dtDistFim.Value, "dd/mm/yy")
        ElseIf (Not IsNull(dtDistInicio.Value)) And (IsNull(dtDistFim.Value)) Then
            XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & _
            "ConsCAPRelSituacaoImoveis.DtDistrato >= " & FunNuloData(dtDistInicio.Value, NomeSgbd) & ""
            XGT_CONJUNCAO = " AND "
            XLT_FILTROPERIODODISTRATO = "Período de Distrato: A partir de " & Format$(dtDistInicio.Value, "dd/mm/yy")
        ElseIf (IsNull(dtDistInicio.Value)) And (Not IsNull(dtDistFim.Value)) Then
            XGT_SELECAO = XGT_SELECAO & XGT_CONJUNCAO & _
            "ConsCAPRelSituacaoImoveis.DtDistrato <= " & FunNuloData(dtDistFim.Value, NomeSgbd) & ""
            XGT_CONJUNCAO = " AND "
            XLT_FILTROPERIODODISTRATO = "Período de Distrato: Até " & Format$(dtDistFim.Value, "dd/mm/yy")
        Else
            XLT_FILTROPERIODODISTRATO = "Período de Distrato: Geral"
        End If
    ...
    Else
      If ChkGaragens.Value = 1 Then
        Set XGR_RELATORIO = XGR_APLICACAO.OpenReport(App.Path + "\Relatorios\RelSituacaoContratoGaragens.rpt")
      Else
        Set XGR_RELATORIO = XGR_APLICACAO.OpenReport(App.Path + "\Relatorios\RelSituacaoContrato.rpt")
      End If
    End If
    ...
    With XGR_RELATORIO.FormulaFields
        .GetItemByName("Periodo").Text = Chr(34) & XLT_FILTROPERIODO & " - " & XLT_FILTROPERIODODISTRATO & Chr(34)
...
Private Sub Form_Load()
  ...
  If XOrigem = "SituacaoContratos" Then
    FraStatus.Visible = False
    Me.Caption = "Relatório Situação dos Contratos"
  Else
    FraStatusContrato.Visible = False
    Me.Caption = "Relatório Situação dos Imóveis"
  End If
