...
Begin VB.Form FrmImportacao 
   Caption         =   "Retorno de Pagamento Eletrônico - ITAÚ"
   ClientHeight    =   1785
   ClientLeft      =   1995
   ClientTop       =   1515
   ClientWidth     =   9120
...
Private Sub CmdConfirmar_Click()
    ...
    If Left(XLT_TEXTO, 26) = "02RETORNO01COBRANCA" & Space(7) Then 'Verifica se é o header de retorno
        
        If Mid(XLT_TEXTO, 77, 18) = "341BANCO ITAU S.A." Then 'Verifica se é o do Banco Itaú
            ...
            If Left(XLT_TEXTO, 1) = "1" And Mid(XLT_TEXTO, 4, 14) = XLT_CGCEMP Then 'Verifica se é registro de transação
                ...
                If Mid(XLT_TEXTO, 109, 2) = "06" Then 'Verifica se é liquidação normal
                    ...
                    'Setar o código do contrato ativo
                    XLT_SQL = "SELECT MAX(cont_cd_contrato) AS cont_cd_contrato FROM Contratos WHERE empd_cd_Empreendimento = '" & XLT_CODEMPREENDIMENTO & _
                    "' AND imov_cd_Imovel = '" & XLT_CODIMOVEL & "' AND empr_cd_Empresa = " & PCodEmpresa
                  
                    SubQOpenRecordset XLO_IMPORTACAO, XLT_SQL, Estatico
              
                    If Not XLO_IMPORTACAO.EOF Then
                        XLT_CODCONTRATO = FunNulo(XLO_IMPORTACAO!cont_cd_Contrato)
                    End If
                    ...
                    If Not XLO_IMPORTACAO.BOF Then
                        ...
                        XGM_MATRIZLOG(3, 1) = XLO_TITULO!titu_vl_Corrigido1
                        XGM_MATRIZLOG(4, 1) = XLO_TITULO!titu_vl_Corrigido2
                        ...
                        'Regsitra o log
                        Call subRegistraLog("TelaExpBancaria", "2", funCriaDescricaoLog(XGM_MATRIZLOG, IMPORTACAO_CAPI, PAGAMENTO_ELETRONICO_ITAU))
                        ...
                        Conexao.Execute "UPDATE Contratos SET cont_dt_Quitacao='" & Format(XLD_DATAPAG, "MM/dd/yyyy") & "'" & _
                        " WHERE empr_cd_empresa = " & XLO_IMPORTACAO!empr_cd_empresa & " AND " & _
                        " empd_cd_Empreendimento= '" & XLO_IMPORTACAO!empd_cd_Empreendimento & "' AND " & _
                        " imov_cd_Imovel='" & XLO_IMPORTACAO!imov_cd_Imovel & "'" & " AND " & _
                        " cont_cd_Contrato='" & XLO_IMPORTACAO!cont_cd_Contrato & "'"
                        End If
                        ...
                        SubQOpenRecordset XLO_FINANCEIRO, "SELECT focl_cd_FornCli,tipl_tx_Descricao FROM ConsCapTitulos WHERE titulo='" & XLT_TITULO & "' AND empr_cd_Empresa=" & PCodEmpresa, Estatico
      ...
      Conexao.CommitTrans
    Else
        MsgBox "Esse não é um arquivo de retorno do Banco Itaú!"
        XLB_ERRO = True
    End If
