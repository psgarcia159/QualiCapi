...
Begin VB.Form TelaManTabCartas 
   Caption         =   "Tabela de Cartas"
   ClientHeight    =   4620
   ClientLeft      =   630
   ClientTop       =   345
   ClientWidth     =   9165
   ...
   Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
      Bindings        =   "TelaManTabCartas.frx":0000
      Height          =   3690
      Left            =   0
      TabIndex        =   8
      Top             =   0
   ...
   Begin MSDataListLib.DataCombo cboLocalizar 
      Bindings        =   "TelaManTabCartas.frx":0015
      Height          =   315
      Left            =   2640
      TabIndex        =   9
      Top             =   3780
   ...
   End
End
Attribute VB_Name = "TelaManTabCartas"
...
Private Sub CmdAlterar_Click()
  XInserir = "A"
  Chave = Adodc1.Recordset("cart_cd_carta").Value
  FrmCartas.Show 1
  If XGB_RESULTADO = True Then SubAtualizarDados
End Sub
...
Private Sub CmdExcluir_Click()
  If MsgBox("Confirma a remoção da carta?", vbQuestion + vbYesNo, "CUIDADO") = vbYes Then
    'Usuário confirmou a remoção do registro
    On Error GoTo RotuloErro
    Conexao.Execute _
      "DELETE FROM cartas WHERE cart_cd_carta= " & Adodc1.Recordset.Fields("cart_cd_Carta")
    SubAtualizarDados
    Quantid = Quantid - 1
...
Private Sub Form_Load()
  ...
  subManutencaoJanelasAtivas "I", "TelaManTabCartas"
      
  'Carrega o recordset das cartas
  subCarregaDados Adodc1, "Cartas", "cart_tx_Descricao", "", _
    "SELECT cart_cd_carta, cart_tx_descricao FROM Cartas"
...
'      Close XLI_ARQUIVO
'      aux = ShellExecute(Me.hwnd, "open", "wordpad.exe", "c:\carta.doc", "", vbNormalFocus)
'  Else
...
'      XFSO_ARQUIVO.CreateTextFile ("carta.txt")
'      Set XFF_ARQUIVO = XFSO_ARQUIVO.GetFile(txtPathCarta.Text)
...
'         XLO_RSCARTA.AddNew
'         XLO_RSCARTA.Fields("cart_tx_Descricao") = txtDescricao.Text
'         XLO_RSCARTA.Fields("cart_tx_texto") = RTBox.Text
'         XLO_RSCARTA.Update
'      Conexao.CommitTrans
'    Else 'ALteracao
'      XLT_SQL = "SELECT * FROM cartas WHERE cart_cd_carta = " & Adodc1.Recordset.Fields("cart_cd_Carta")
...
'      Conexao.BeginTrans
'        XLO_RSCARTA.Fields("cart_tx_Descricao") = txtDescricao.Text
'        XLO_RSCARTA.Fields("cart_tx_texto").Value = RTBox.Text
