...
Begin VB.Form TelaInicial 
   Caption         =   "Pagamento Eletrônico - Sudameris"
   ClientHeight    =   5370
   ClientLeft      =   765
   ClientTop       =   1800
   ClientWidth     =   7440
...
Private Sub Form_Load()
    ...
    XGT_NomeAplic = "EXPSUDAMERIS"
    XGT_IdAplic = "EXPSUDAMERIS-SIREMADUSPXE"
    ...
    Diretorio = Mid(App.Path, 1, (Len(App.Path) - Len("\QualiCapi\ExpSudameris")))
    
    'Verifica a versão do cliente no banco de dados
    SubQOpenRecordset XLO_VERIFICA, "SELECT sist_vl_versao FROM Sistema WHERE sist_tx_nome = 'QUALICAPI - EXPSUDAMERIS'", Estatico
    ...
    If Not XLO_VERIFICA.BOF Then
        ...
        If CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) > CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            Conexao.Execute _
                ...
                " WHERE sist_tx_nome = 'QUALICAPI - EXPSUDAMERIS'"
        ...
        ElseIf CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) < CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            res = MsgBox("Máquina desatualizada! Deseja atualizar o ExpSudameris nesta máquina?", VbMsgBoxStyle.vbYesNo + vbQuestion, "Aviso")
            
            If (res = vbYes) Then
                Call Shell(Diretorio & "\Atualizador\Atualizador.exe QualiCapi ExpSudameris", vbNormalFocus)
                End
            Else
