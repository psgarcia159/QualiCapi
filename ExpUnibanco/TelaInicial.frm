...
Begin VB.Form TelaInicial 
   Caption         =   "Pagamento Eletrônico - UNIBANCO"
   ClientHeight    =   5370
   ClientLeft      =   765
   ClientTop       =   1800
   ClientWidth     =   7440
...
Private Sub Form_Load()
    ...
    XGT_NomeAplic = "EXPUNIBANCO"
    XGT_IdAplic = "EXPUNIBANCO-OCNABINUPXE"
    ...
    Diretorio = Mid(App.Path, 1, (Len(App.Path) - Len("\QualiCapi\ExpUnibanco")))
    
    'Verifica a versão do cliente no banco de dados
    SubQOpenRecordset XLO_VERIFICA, "SELECT sist_vl_versao FROM Sistema WHERE sist_tx_nome = 'QUALICAPI - EXPUNIBANCO'", Estatico
    ...
    If Not XLO_VERIFICA.BOF Then
        ...
        If CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) > CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            Conexao.Execute _
                ...
                " WHERE sist_tx_nome = 'QUALICAPI - EXPUNIBANCO'"
        ...
        ElseIf CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) < CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            res = MsgBox("Máquina desatualizada! Deseja atualizar o ExpUnibanco nesta máquina?", VbMsgBoxStyle.vbYesNo + vbQuestion, "Aviso")
            
            If (res = vbYes) Then
                Call Shell(Diretorio & "\Atualizador\Atualizador.exe QualiCapi ExpUnibanco", vbNormalFocus)
                End
            Else
