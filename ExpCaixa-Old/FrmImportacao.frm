...
Begin VB.Form FrmImportacao 
   ...
   Begin Threed.SSPanel PanConta 
      ...
      Begin MSDataListLib.DataCombo cboCCorrente 
         Bindings        =   "FrmImportacao.frx":0000
         Height          =   330
         Left            =   1545
         TabIndex        =   3
         Top             =   300
    ...
    End If
        ...
        If Mid(XLT_TEXTO, 2, 1) = "2" Then 'Verifica se é o header de retorno
            If Trim(Mid(XLT_TEXTO, 77, 3)) = 104 Then 'Verifica se é o do Banco Caixa
                                ...
                                If IsNull(XLO_IMPORTACAO!titu_dt_Pagamento) Then
                                    ...
                                    If Not IsNull(XLO_IMPORTACAO!moed_cd_Moeda3) Then
                                        If XLO_IMPORTACAO!moed_cd_Moeda3 = XLO_IMPORTACAO!moed_cd_Moeda1 Then
                                            XLF_PREVISTO = Format(ModQualiCapiPagamento.funCalculaCorrecaoMonetaria(XLO_IMPORTACAO, 1, XLO_IMPORTACAO!titu_dt_Vencimento), "Standard")
                                        Else
                                            XLF_PREVISTO = Format(ModQualiCapiPagamento.funCalculaCorrecaoMonetaria(XLO_IMPORTACAO, 2, XLO_IMPORTACAO!titu_dt_Vencimento), "Standard")
                                        End If
                                    Else
                                    ...
                                    If IsNull(XLO_IMPORTACAO!moed_cd_Moeda3) Then
                                        XLT_TIPOPAG = ModQualiCapiPagamento.funGeraTipoPag(0, XLF_VALORPAGO, XLF_VALORPAGO - XLF_JUROS + XLF_DESCONTO, XLF_PREVISTO, XLO_IMPORTACAO!titu_dt_Vencimento, XLD_DATAPAG)
                                    Else
                                        XLT_TIPOPAG = ModQualiCapiPagamento.funGeraTipoPag(1, XLF_VALORPAGO, XLF_VALORPAGO - XLF_JUROS + XLF_DESCONTO, XLF_PREVISTO, XLO_IMPORTACAO!titu_dt_Vencimento, XLD_DATAPAG)
                                    End If
                                    ...
                                    'Registra o log
                                    Call subRegistraLog("TelaExpBancaria", "2", funCriaDescricaoLog(XGM_MATRIZLOG, EXPORTACAO_CAPI, PAGAMENTO_ELETRONICO_CAIXA))
                                    ...
                                    ModQualiCapiPagamento.subGravaFinanceiro XLO_TITULO, "PAGAR", XLO_FINANCEIRO!focl_cd_FornCli, XLO_FINANCEIRO!tipl_tx_Descricao, XLT_NATUREZAPLANO
                ...
                Conexao.CommitTrans
            Else
                MsgBox "Esse não é um arquivo de retorno do Banco Caixa!"
                XLB_ERRO = True
            End If
