VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} RptExtratoContratoAnaliticoReal 
   ClientHeight    =   13440
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   14010
   OleObjectBlob   =   "RptExtratoContratoAnaliticoReal.dsx":0000
End
Attribute VB_Name = "RptExtratoContratoAnaliticoReal"
...
'Dim XFB_COTACAOFALTANDO As Boolean
...
AddReportVariable crRVNumber, "SOMAVALORHISTORICO"
...
Private Sub SecDetalhe_Format(ByVal pFormattingInfo As Object)
  ...
  Dim XLT_NOMEMOEDATEMP As String 'Nome da moeda quando temporário quando há a troca pelo indexador extra
  ...
  If XLO_CAMPOS.GetItemByName("Pago").Value = "N" Then
    ...
    'Juros da CEF
    XLF_VALORPREVISTO = XLF_VALORCORRIGIDO + FunCalculoJurosCEF(XLO_CAMPOS, XLD_DATABASE, XLF_VALORCORRIGIDO)
  ...
  Else
    ...
    XLF_VALORPREVISTO = XLF_VALORCORRIGIDO - XLF_DESAGIO
  ...
  If XLI_INDEXADOR = 0 Then
    SetReportVariableValue "SOMAVALORHISTORICO", (FunNuloVal(GetReportVariableValue("SOMAVALORHISTORICO")) + CDbl(Format(XLF_VALORTITULO, "standard")))
  End If
  
  '----------------------------- Campos Texto para pagos e não pagos ----------------
  
  If XLI_INDEXADOR = 0 Then
    TxtMoeda.SetText "REAL"
  Else
  ...
  TxtObservacao.SetText FunObservacao2(XLO_CAMPOS, XLD_DATABASE)
...
Private Sub SecFooterCliente1_Format(ByVal pFormattingInfo As Object)
  ...
  If XFB_IMPRIMIU = False Then
    ...
    VFV_VALORES(0, 2) = GetReportVariableValue("TOTALDEBITOATUALIZADO")
    VFV_VALORES(0, 3) = GetReportVariableValue("TOTALLIQUIDADOATUALIZADO")
  ...
  Else
      ...
      If VFV_SOMATORIO(XFI_CONT, 0) = Left(XFT_TITULOANTERIOR, 12) Then
        While XFI_CONT < VFV_SOMATORIO.Count(1)
          ...
          Else
            ...
            TxtSomaValorPago.SetText FunTrataZero(Format(VFV_SOMATORIO(XFI_CONT, 2), "standard"))
            XFI_CONT = XFI_CONT + 1
          End If
        Wend
      End If
    ...
    TxtSomaValorHistorico.SetText Format(GetReportVariableValue("SOMAVALORHISTORICO"), "standard")
  ...
  SetReportVariableValue "TOTALDEBITOATUALIZADO", 0
  SetReportVariableValue "TOTALLIQUIDADOATUALIZADO", 0
  SetReportVariableValue "SOMAVALORHISTORICO", 0
...
Private Sub SecFooterCliente3_Format(ByVal pFormattingInfo As Object)
  ...
  'Descarrega os vetores com os totais atualizados
  If XFB_IMPRIMIU = True Then
    For XFI_CONT = 0 To VFV_SOMATORIO.Count(1) - 1
      If VFV_SOMATORIO(XFI_CONT, 0) = Left(XFT_TITULOANTERIOR, 12) Then
        While XFI_CONT < VFV_SOMATORIO.Count(1)
          If VFV_SOMATORIO(XFI_CONT, 0) <> Left(XFT_TITULOANTERIOR, 12) Then
            XFI_CONT = VFV_SOMATORIO.Count(1) + 1
          Else
            TxtTotalDebitoAualizado.SetText FunTrataZero(Format(FunNuloVal(VFV_SOMATORIO(XFI_CONT, 3)), "standard"))
            TxtValorImovel.SetText FunTrataZero(Format(FunNuloVal(VFV_SOMATORIO(XFI_CONT, 3) + FunNuloVal(VFV_SOMATORIO(XFI_CONT, 4))), "standard"))
            TxtTotalLiquidadoAtualizado.SetText FunTrataZero(Format(FunNuloVal(VFV_SOMATORIO(XFI_CONT, 4)), "standard"))
            XFI_CONT = XFI_CONT + 1
          End If
        Wend
      End If
