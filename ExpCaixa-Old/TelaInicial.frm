...
Begin VB.Form TelaInicial 
   Caption         =   "Pagamento Eletrônico - CAIXA"
   ClientHeight    =   5370
   ClientLeft      =   765
   ClientTop       =   1800
   ClientWidth     =   7500
   LinkTopic       =   "Form1"
   Picture         =   "TelaInicial.frx":0000
   ScaleHeight     =   9025.634
   ScaleMode       =   0  'User
   ScaleWidth      =   7500
   Begin VB.CommandButton btnImportar 
      Height          =   840
      Left            =   2265
      Picture         =   "TelaInicial.frx":109AB
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   2790
      Width           =   1100
...
Private Sub Form_Load()
    ...
    XGT_NomeAplic = "EXPCAIXA"
    XGT_IdAplic = "EXPCAIXA-AXIACPXE"
    subTestaProtecao XGT_NomeAplic, XGT_IdAplic, XGT_DirPadrao
    XGT_NomeAplic = "EXPCAIXA"
    lblEmpresa.Caption = PFantasia
    TelaInicial.lblVersao.Caption = "R " & Format(App.Major, "#0") & "." & Format(App.Revision, "00")
    
    'ATUALIZAÇÃO DO SISTEMA
    
    Diretorio = Mid(App.Path, 1, (Len(App.Path) - Len("\QualiCapi\ExpCAIXA")))
    
    'Verifica a versão do cliente no banco de dados
    SubQOpenRecordset XLO_VERIFICA, "SELECT sist_vl_versao FROM Sistema WHERE sist_tx_nome = 'QUALICAPI - EXPCAIXA'", Estatico
    ...
    If Not XLO_VERIFICA.BOF Then
        ...
        If CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) > CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            Conexao.Execute _
                ...
                " WHERE sist_tx_nome = 'QUALICAPI - EXPCAIXA'"
        ...
        ElseIf CDbl(Format(App.Major, "#0") & "," & Format(App.Revision, "00")) < CDbl(XLO_VERIFICA!sist_vl_versao) Then
            ...
            res = MsgBox("Máquina desatualizada! Deseja atualizar o ExpCAIXA nesta máquina?", VbMsgBoxStyle.vbYesNo + vbQuestion, "Aviso")
            
            If (res = vbYes) Then
                Call Shell(Diretorio & "\Atualizador\Atualizador.exe QualiCapi ExpCAIXA", vbNormalFocus)
                End
            Else
